...
function Load_Template_Scripts(){
    ...
    if ( is_page_template('trial-free.php') ) {
		wp_enqueue_script('jquery-validate-min', 
		get_stylesheet_directory_uri() . '/assets/js/jquery.validate.min.js', 
		array( 'jquery' ));
		wp_enqueue_script('trial_general', 
		get_stylesheet_directory_uri() . '/assets/js/trial_general.js', 
		array( 'jquery' ));
		wp_localize_script( 'trial_general', 'PT_Ajax', array(
...
function add_experience_now(){
	...
	$action  = $_POST['add_experience_now '];

	
	if($wpdb->insert('wp_expereience_now2',array(
	'firstname'=>$firstname,
	'lastname'=>$lastname,
	'email'=>$email,
...
add_action('wp_ajax_add_experience_now', 'add_experience_now');
add_action('wp_ajax_nopriv_add_experience_now', 'add_experience_now'); // not really needed